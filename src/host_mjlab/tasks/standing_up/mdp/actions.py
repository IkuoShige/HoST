...
class JointPositionOffsetActionCfg(BaseActionCfg):
  ...
  """Clip raw actions to [-clip_actions, clip_actions] before scaling.
  Prevents noise std explosion from causing extreme joint offsets in MuJoCo."""
...
class JointPositionOffsetAction(BaseAction):
  ...
  def process_actions(self, actions: torch.Tensor):
    ...
    clipped = torch.clip(self._raw_actions, -self._clip_actions, self._clip_actions)
    self._processed_actions = clipped * self._scale
