...
def feet_contact_balance(
  ...
  left_foot_body: str = "l_ankle_roll_link",
  right_foot_body: str = "r_ankle_roll_link",
  asset_cfg: SceneEntityCfg = _DEFAULT_ASSET_CFG,
) -> torch.Tensor:
  """Reward balanced contact forces between feet.

  Uses cfrc_ext (external contact force on bodies) directly from MuJoCo,
  avoiding the need for a separate contact sensor abstraction.
  ...
  # cfrc_ext shape: (num_envs, nbody, 6) — columns 3:6 are linear force (fx, fy, fz).
  left_fz = torch.abs(env.sim.data.cfrc_ext[:, body_ids[left_ids[0]], 5])
  right_fz = torch.abs(env.sim.data.cfrc_ext[:, body_ids[right_ids[0]], 5])
...
def dof_vel_limits(
  ...
  """Penalize DOF velocities exceeding soft limits.

  Matches HoST_v2: sum of (|vel| - limit*soft_ratio) clipped to [0, 1].

  Args:
    env: The environment.
    soft_ratio: Fraction of hard limit used as soft limit. Default 0.9.
    max_velocity: Maximum joint velocity from URDF. Default 5.0 rad/s.
    asset_cfg: Asset configuration.
  ...
  asset: Entity = env.scene[asset_cfg.name]
  vel = asset.data.joint_vel[:, asset_cfg.joint_ids]
  soft_limit = max_velocity * soft_ratio
  return torch.sum(
    (torch.abs(vel) - soft_limit).clip(min=0.0, max=1.0), dim=1
  )


def knee_deviation(
  env: ManagerBasedRlEnv,
  max_abs_threshold: float = 1.65,
  min_val_threshold: float = -0.6,
  asset_cfg: SceneEntityCfg = _DEFAULT_ASSET_CFG,
) -> torch.Tensor:
  """Penalize knee (calf) joint deviations.

  Matches HoST_v2: binary penalty if |knee| > 1.65 or knee < -0.6.

  Args:
    env: The environment.
    max_abs_threshold: Maximum absolute value threshold. Default 1.65.
    min_val_threshold: Minimum value threshold. Default -0.6.
    asset_cfg: Asset configuration.
  ...
  joint_ids, _ = asset.find_joints(".*calf_joint")
  if not joint_ids:
    return torch.zeros(env.num_envs, device=env.device)

  pos = asset.data.joint_pos[:, joint_ids]
  violated = (
    (torch.max(torch.abs(pos), dim=-1)[0] > max_abs_threshold)
    | (torch.min(pos, dim=-1)[0] < min_val_threshold)
  )
  return violated.float()
...
def target_ang_vel_xy(
  ...
  """Penalize XY angular velocity after standing up.

  Matches HoST_v2 _reward_ang_vel_xy: only active above phase3 height.
