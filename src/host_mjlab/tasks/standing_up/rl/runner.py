...
class StandingUpRunner(MjlabOnPolicyRunner):
  ...
  def load(
    ...
    if infos and "curriculum_state" in infos:
      ...
      # Helper to resize tensor to current num_envs.
      def resize_to_num_envs(tensor: torch.Tensor) -> torch.Tensor:
        """Resize saved tensor to current num_envs (broadcast or slice)."""
        tensor = tensor.to(device)
        if tensor.shape[0] == num_envs:
          return tensor
        elif tensor.shape[0] == 1:
          # Broadcast single value to all envs.
          return tensor.expand(num_envs, *tensor.shape[1:]).clone()
        elif num_envs == 1:
          # Take mean for single env (play mode).
          return tensor.mean(dim=0, keepdim=True)
        else:
          # Different num_envs: use mean value for all.
          mean_val = tensor.mean(dim=0, keepdim=True)
          return mean_val.expand(num_envs, *tensor.shape[1:]).clone()

      # Restore force magnitude.
      # Dynamic attributes are set by curriculum manager.
      if "force_magnitude" in curriculum_state:
        force = resize_to_num_envs(curriculum_state["force_magnitude"])
        unwrapped.force_magnitude = force  # type: ignore[attr-defined]

      # Restore action rescale.
      if "action_rescale" in curriculum_state:
        action_scale = resize_to_num_envs(curriculum_state["action_rescale"])
      ...
      if "max_head_height" in curriculum_state:
        max_height = resize_to_num_envs(curriculum_state["max_head_height"])
