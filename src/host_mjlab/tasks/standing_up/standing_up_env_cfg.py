...
from host_mjlab.tasks.standing_up import mdp
from host_mjlab.tasks.standing_up.managers import (
...
def make_standing_up_env_cfg() -> ManagerBasedRlEnvCfg:
  ...
  # HoST observation: 43 dim x 6 history = 258 features.
  ...
  rewards: dict[str, GroupedRewardTermCfg] = {
    # Dense progress rewards (compensate for MuJoCo's softer contact dynamics).
    # Not in original HoST_v2, but replaces the implicit gradient that PhysX provides.
    ...
    # Task rewards (product aggregation) -- matches HoST_v2 exactly.
    ...
    # Regularization rewards (additive) -- matches HoST_v2 exactly.
    ...
    "regu_dof_vel_limits": GroupedRewardTermCfg(
      func=mdp.dof_vel_limits,
      weight=-1.0,
      group="regu",
    ),
    # Style rewards (additive) -- matches HoST_v2 exactly.
    ...
    "style_knee_deviation": GroupedRewardTermCfg(
    ...
    "style_feet_min_distance": GroupedRewardTermCfg(
      func=mdp.feet_min_distance,
      weight=-50.0,
      group="style",
      params={"min_distance": 0.10},
    ),
    "style_style_ang_vel_xy": GroupedRewardTermCfg(
      func=mdp.style_ang_vel_xy,
      weight=1.0,
      group="style",
      params={"decay_rate": 2.0, "phase1_height": 0.25},
    ),
    ...
    "style_feet_contact_balance": GroupedRewardTermCfg(
      ...
      params={
        "decay_rate": 5.0,
        "phase2_height": 0.25,
        "left_foot_body": "l_ankle_roll_link",
        "right_foot_body": "r_ankle_roll_link",
      },
    ),
    ...
    "target_ang_vel_xy": GroupedRewardTermCfg(
      ...
      weight=10.0,
      group="target",
      params={"decay_rate": 2.0, "phase3_height": 0.34},
    ),
  ...
  return ManagerBasedRlEnvCfg(
    scene=SceneCfg(
      terrain=TerrainImporterCfg(terrain_type="plane"), num_envs=3072
    ),  # 4096
    ...
    sim=SimulationCfg(
      nconmax=150,  # Per-env contact buffer (Pi robot needs ~50, add margin).
      njmax=1500,  # Per-env constraint buffer (needs ~491, add margin for later iterations). 1420
      mujoco=MujocoCfg(
        timestep=0.002,  # Reduced for stability.
        iterations=50,  # Increased to match PhysX contact stiffness.
        ls_iterations=100,  # Increased for better constraint convergence.
        ccd_iterations=100,  # Increased for fast-moving collision detection.
      ),
    ),
